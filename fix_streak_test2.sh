...
import 'package:app_islamic_v2/core/dda_engine.dart';
import 'dart:io';
...
void main() {
  ...
  setUp(() async {
    ...
    final db = await dao.getStreak('M01'); // init db
  });
  ...
  test('publish 5 salat_completed events (different prayer_names) -> streak for M01 = 1', () async {
    ...
    final notifier = container.read(trackerNotifierProvider);
  ...
  test('Pause for Mercy active -> streak not decremented when salat missed', () async {
    ...
    await dao.upsertStreak('M01', 1, 1, DateTime.now().subtract(const Duration(days: 2)).toIso8601String());
